...
/* the above . is to allow the syntax $.xxx.xxx */
...
#define endfile_block       rule_block[ENDFILE]
	...
	prog_block->lasti->target_jmp = ip_rec;   /*Jump to ip_rec only if there are still input records*/
...
retry:
	...
	case '.':
		...
		pushback();
		if (! isdigit(c))
			return lasttok = '.';  /* this allows us to use "." in $.xxx */
		else
			c = '.';
		/* FALL THROUGH */
	case '0':
...
list_create(INSTRUCTION *x)
{
	...
	l->nexti = x;
	l->lasti = x;  /*Last instruction in the list*/
	return l;
}
