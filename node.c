...
/* make_bin_node --- make a binary node */

NODE *
make_bin_node(const char *s, size_t len, int flags)
{
        NODE *r;
	getnode(r);
	r->type = Node_val;
	r->numbr = 0;
	r->flags = flags & NO_FREE? (BINCUR|BINARY) : (MALLOC|BINCUR|BINARY);
	r->valref = 1;
	r->stfmt = -1;
	r->wstptr = NULL;
	r->wstlen = 0;
	...
	if ((flags & ALREADY_MALLOCED) != 0 ) {
	        ...
	        if (len == 0) {
		        r->stptr = NULL;
	                ...
	                emalloc(r->stptr, char *, len, "make_bin_node");
...
{
	NODE *r;
	getnode(r);         /* pyang: get a node from the freelist. */
	r->type = Node_val;
	r->numbr = 0;
	...
	if ((flags & ALREADY_MALLOCED) != 0)
		...
		emalloc(r->stptr, char *, len + 2 /* pyang: why plus 2, isn't 1 enough? */, "make_str_node");
		memcpy(r->stptr, s, len);
	}
	r->stptr[len] = '\0';
	...
	if ((flags & SCAN) != 0) {	/* scan for escape sequences */
		...
		for (pf = ptm = r->stptr; pf < end;) {
			...
			c = *pf++;
			if (c == '\\') {
				c = parse_escape(&pf); //parse_escape will advance pf again, so whatever follows '\\' will be overwritten later.
				if (c < 0) {
					if (do_lint)
...
const char* get_node_str(const NODE *n)
{
    return n->stptr;
}
